...
function renderLodgeModal(){
 ...
 $('#lodgeModalContent').innerHTML=`<div class="modal-lodge-image" style="background-image:url('${l.img}')"></div><div class="modal-lodge-body"><span class="eyebrow">${l.country} · ${l.city}</span><h2>${l.name}</h2><div class="modal-meta">★ ${l.rating} ·lodge profile</div><p>${l.desc}</p><div class="reward-panel"><div><span>Simulated review offer</span><br><small>Educational display only — no payout is promised.</small></div><strong>${money(l.offer)}</strong></div>${action}</div>`;
